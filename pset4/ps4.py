...
def r_squared(y, estimated):
    ...
    n = len(y)
    mean = sum(y) / n
    numerator = sum([(y[i] - estimated[i]) ** 2 for i in range(n)])
    denominator = sum([(y[i] - mean) ** 2 for i in range(n)])
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        pylab.plot(x, est, "r", label="Model: " + str(model))
        pylab.legend(loc="best")
        pylab.title("Model: " + str(model) + "\n" + "R^2: " + str(r2))
        pylab.show()
